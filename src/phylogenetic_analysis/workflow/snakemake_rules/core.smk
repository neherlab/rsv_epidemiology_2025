...
rule filter_lab:
    ...
    params:
        min_coverage=lambda w: f'{w.build_name}_coverage>.05',
        min_length=0,
...
rule combine_samples:
    input:
        subsamples=lambda w: (
            [
                rules.filter_lab.output.sequences,
                rules.filter_os.output.sequences,
            ]
            if "min_date" in config["filter"]["resolutions"][w.resolution]
            else [rules.filter_lab.output.sequences]
        ),
    output:
